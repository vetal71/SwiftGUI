(*==============================================================================
  Программа  : Система ведения договоров
  Модуль     : 
  Авторы     : Смотрин А.А.
  Назначение :
==============================================================================*)
...
const
  SWIFT_MrkRefId = 30001;                                       // сообщение отправлено
  cMessagesLinkKind = 70000;                                    // тип связи сделки с сообщением
  cMessagesLinkByRefKind = 70001;                               // связь сообщения с сообщением по референсу
  cMessagesToMessagesLinkKind = 70003;                          // связь сообщения с родительским сообщением
  PaySystems: array[0..1] of String = ('SWIFT', 'TELEX');       // платежные системы
  ItemPathName = 'Внешнеэкономические операции';                // Ветка в НСИ

  cnSwiftItemPath = 'Архив swift сообщений';                    // Ветка архива SWIFT сообщений в НСИ
...
const
  {символы, допустимые в использовании SWIFT}
  SWIFTPermissible=['A'..'Z','a'..'z','0'..'9','/','-','?',':','(',')','.',#44,#39,'+',' '];

(*
//сохранить сообщение в SWIFTMSGARCHIVE
...
//удаляет из строки кавычки и двойные кавычки
function CopyNotQuote(str: String): String;
//дополнительное преобразование строки для исходящих сообщений
function CopySwiftAdd(const str: String): String;
//замена/дополнение '.' на ',' в строке
function ReplacePointStr(str: String): String;
*)
//Проверка на наличие слэша '/' в первой и последней позиции поля; на наличие двух последовательных слэшей '//' в поле
//Применимо в основном для :20: и :21: поля
function CheckingForSlashes(const StrValid: string; const FieldName: string): string;
(*
//Проверка поля на допустимую длину и наличие кириллических символов
...
// Возвращает информацию о финансовой организации для МР)
// необходима, что бы наименования банка соответствовали международному справочнику
function GetBankInfoForMR(const ID: Integer; const sBIC, sSW: String; const bR: Boolean = False): SubjectSWIFTInfo;

////////////////////////////////////////////////////////////////////////////////

// Возвращает информацию о субъекте
...
// Возвращает информацию о внешнем счете
...
// синхронизация родителей swift сообщений
...
{возвращает признак наличия в строке недопустимые для S.W.I.F.T. символов}
function IsSWIFTPermissible(const str: String; out strError: String): Boolean;


// Проверяет необходимость авторизации данного типа SWIFT сообщения
// по соответсвию маске заданной в НСИ
...
begin
  ...
  //значение не должно начинаться со слэша
  if (eStrValid > '') and (Copy(eStrValid,1,1) = '/') then
    sErr := sErr + #13 + 'поле ' + FieldName + ' не должно начинаться со слэша "/"';

  //заканчиваться слэшем
  if (eStrValid > '') and (Copy(eStrValid, length(eStrValid), 1)='/') then
    sErr := sErr + #13 + 'поле ' + FieldName + ' не должно заканчиваться слэшем "/"';

  //содержать внутри двойной слэш
  if (eStrValid > '') and Assigned(AnsiStrPos(PChar(eStrValid), PChar('//'))) then
    sErr  := sErr + #13 + 'поле ' + FieldName + ' не должно содержать внутри двойной слэш "//"';
...
begin
   if (length(StrValid) > ValidLength) then
     sErr := sErr+#13+Format('%s'#$9'%s - длина (%d) ==> допустимая длина (%d)',[FieldName, StrValid, length(StrValid), ValidLength]);

   if IsTrans(StrValid) then
     sErr := sErr+#13+Format('%s'#$9'%s ==> недопустимые символы',[FieldName, StrValid]);
...
begin
  ...
  with TMyQuery.Create(nil) do try
    ...
    if not(EOF) then begin
      ...
      raise ENoRecordFound.Create(Format('AccInfoById: Внешний счет не найден (%s %d).', [Acc, Curr]));
    end;
  finally
    Free;
  end;
...
begin
  with TMyQuery.Create() do try
    // связь между aSwiftMsgEntID(sub) и родителем aEntID(top)
    ExecQuery(SQLText, [aSwiftMsgEntID, aActID, cMessagesLinkKind, aEntID]);
    // связь между aEntID(sub) и родителем aSwiftMsgEntID(top)
